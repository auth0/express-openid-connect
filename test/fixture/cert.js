...
// JWK representation of the test RSA key
const jwk = {
  e: 'AQAB',
...
module.exports.key = privateKey.export({ type: 'pkcs8', format: 'pem' });
module.exports.kid = kid;

// Synchronous token generation using Node.js crypto directly (like jose v2 did internally)
// This creates a simple JWT without using jose's async APIs
function createJWT(payload, key, header = {}) {
  const headerObj = { alg: 'RS256', ...header };
  const encodedHeader = Buffer.from(JSON.stringify(headerObj)).toString(
    'base64url',
  );
...
module.exports.makeIdToken = (payload = {}) => {
  ...
  return createJWT(fullPayload, privateKey, { kid });
};
...
module.exports.makeLogoutToken = ({ payload = {}, sid, sub, secret } = {}) => {
  ...
  if (secret) {
    ...
    const encodedHeader = Buffer.from(
      JSON.stringify({ alg: 'HS256', typ: 'logout+jwt' }),
    ).toString('base64url');
    ...
    return createJWT(claims, privateKey, { typ: 'logout+jwt' });
  }
};
