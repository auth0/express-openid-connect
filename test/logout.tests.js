...
describe('logout route with enterpriseConnect', async () => {
  ...
  it('should route through the classic /v2/logout path for an auth0.com issuer, same as non-EC (no special branching)', async () => {
    server = await createServer(
      auth({
        ...defaultConfig,
        issuerBaseURL: 'https://test.eu.auth0.com',
  ...
  it('should omit id_token_hint when the SDK holds no session, matching real Enterprise Connect usage', async () => {
    server = await createServer(
      auth({ ...defaultConfig, enterpriseConnect: true, idpLogout: true }),
    );

    // No login() call: this mirrors real Enterprise Connect usage, where
    // afterCallback returning undefined/null means the SDK's own session
    // is never written in the first place.
    const {
      response: {
        headers: { location },
      },
    } = await logout(request.jar());
    const url = new URL(location);
...
describe('logout route federated shorthand', async () => {
  let server;
  ...
  it('should support federated as a top-level shorthand for logoutParams.federated, without enterpriseConnect', async () => {
    const router = auth({
      ...defaultConfig,
      idpLogout: true,
      routes: { logout: false },
  ...
  it('should let an explicit logoutParams.federated override the top-level shorthand', async () => {
    const router = auth({
      ...defaultConfig,
      idpLogout: true,
      routes: { logout: false },
    ...
    server = await createServer(router);
    router.get('/logout', (req, res) =>
      res.oidc.logout({ federated: true, logoutParams: { federated: 'no' } }),
    );
    ...
    const url = new URL(location);
    assert.equal(url.searchParams.get('federated'), 'no');
  });
});
