...
class TransientCookieHandler {
  ...
  async store(
    ...
    {
      const cookieValue = await generateCookieValue(key, value, this.currentKey);
  ...
  async getOnce(key, req, res) {
    ...
    let value = await getCookieValue(key, req[COOKIES][key], this.keyStore);
