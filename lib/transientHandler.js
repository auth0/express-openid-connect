...
const HEADER = 'eyJhbGciOiJIUzI1NiIsImI2NCI6ZmFsc2UsImNyaXQiOlsiYjY0Il19';
...
const generateSignature = (cookie, value, key) => {
  ...
  return JWS.sign.flattened(payload, key, { alg: 'HS256', b64: false, crit: ['b64'] }).signature;
};
const verifySignature = (cookie, value, signature, keystore) => {
  try {
    return !!JWS.verify(flattenedJWSFromCookie(cookie, value, signature), keystore, { algorithm: 'HS256', crit: ['b64'] });
  } catch (err) {
    return false;
  }
};
...
class TransientCookieHandler {
  constructor({ secret, session, legacySameSiteCookie }) {
    ...
    this.secureCookieConfig = session && session.cookie && session.cookie.secure;
  ...
  store (key, req, res, { sameSite = 'None', value = this.generateNonce() } = {}) {
    ...
    const basicAttr = { httpOnly: true, secure: typeof this.secureCookieConfig === 'boolean' ? this.secureCookieConfig : req.secure };

    {
      ...
      res.cookie(key, cookieValue, { ...basicAttr, sameSite, secure: isSameSiteNone ? true : basicAttr.secure });
    }
