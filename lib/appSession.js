...
module.exports = (config) => {
    ...
    for (const key of keystore) {
      try {
        ...
        return {
          ...
          cleartext: new TextDecoder().decode(plaintext),
        };
      } catch (err) {
        lastError = err;
        // Try next key
  ...
  class CookieStore {
    async get(idOrVal) {
      const { protected: header, cleartext } = await decrypt(idOrVal);
      return {
        header,
  ...
  return async (req, res, next) => {
    ...
    try {
      if (req[COOKIES].hasOwnProperty(sessionName)) {
        ...
        existingSessionValue = await store.getCookie(req);
      ...
      if (existingSessionValue) {
        ...
        if (!sessionData || typeof sessionData !== 'object') {
          ...
          if (!header || typeof header !== 'object') {
            ...
            // A 30s leeway guards against clock skew between the SDK and the Auth0 platform.
            if (data.sessionExpiresAt) {
              assert(
                data.sessionExpiresAt - 30 > epoch(),
