      ...
      if (existingSessionValue) {
        ...
        if (!sessionData || typeof sessionData !== 'object') {
          ...
          if (!header || typeof header !== 'object') {
            ...
            // MCD Domain Migration Support
            // Sessions remain bound to their original issuer - no cross-issuer validation needed.
            // Only reject sessions that are missing the issuer field in MCD mode (pre-MCD legacy sessions).
            if (typeof config.issuerBaseURL === 'function') {
              if (data.issuer) {
                // Session has issuer - ACCEPT it (session remains bound to original issuer)
                // New authentication on a different domain will create a new session with new issuer
                debug('MCD session accepted, bound to issuer: %s', data.issuer);
              } else {
                // Session without issuer in MCD mode - reject (pre-MCD legacy session)
                // User must re-authenticate to get a session with issuer field
                assert.fail(
                  'session missing issuer field in MCD mode - re-authentication required',
                );
              }
            }
            // Static issuerBaseURL mode: no issuer validation needed (backward compatible)
