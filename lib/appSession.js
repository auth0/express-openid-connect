...
module.exports = (config) => {
  ...
  function setCookie(
    ...
    if (!req[sessionName] || !Object.keys(req[sessionName]).length) {
      ...
      const dataToEncrypt = {
        s: req[sessionName],
        t: TokenSets.getAll(req),
      };

      const value = encrypt(JSON.stringify(dataToEncrypt), {
        iat,
        uat,
        exp,
        v: LATEST_COOKIE_VERSION, // see CookieStore.get() for versions
      });
  ...
  class CookieStore {
    async get(idOrVal) {
      ...
      // no version => <session> directly in the object root
      if (header.v === undefined) {
        return {
          header,
          data: parsedClearText,
          tokenSets: [],
        };
      }

      switch (header.v) {
        // v2 => `{ s: <session>, t: <tokensets> }`
        case 2:
          return {
            header,
            data: parsedClearText.s,
            tokenSets: parsedClearText.t,
          };

        default:
          throw new TypeError('Unknown cookie version number');
      }
    }
    ...
    async set(
      ...
      if (hasCurrentSession) {
        await this._set(req[REGENERATED_SESSION_ID] || id, {
          ...
          tokenSets: TokenSets.getAll(req),
          cookie: {
            expires: exp * 1000,
  ...
  return async (req, res, next) => {
    ...
    try {
      ...
      if (existingSessionValue) {
        const { header, data, tokenSets } = await store.get(existingSessionValue);
        ({ iat, uat, exp } = header);
        ...
        // Special case: a session that was created in an older version will have
        // an empty tokenset list but might have a valid tokenset in the session.
        // In that case, append it to the list (excess properties don't matter).
        if (tokenSets.length === 0 && req[sessionName].id_token) {
          TokenSets.append(req, req[sessionName]);
        }
