...
// Polyfill fetch and crypto for Node.js 18 compatibility
if (!globalThis.fetch) {
...
// Ensure crypto is available globally for openid-client v6
...
async function get(config) {
  ...
  const customFetchFn = async (url, options = {}) => {
    ...
    // Add timeout - use default of 10 seconds for CI compatibility if not specified
    const timeoutMs = config.httpTimeout || 10000;
  ...
  try {
    ...
    // Restore original global fetch for test isolation
