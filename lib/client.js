...
async function get(config) {
  ...
  if (config.clientAssertionSigningKey) {
    const jwkOpts = { use: 'sig' };
    if (config.clientAssertionSigningAlg) {
      jwkOpts.alg = config.clientAssertionSigningAlg;
    }
    const jwk = JWK.asKey(config.clientAssertionSigningKey, jwkOpts).toJWK(
      true
    );
    jwks = { keys: [jwk] };
  }
