...
async function createClient(config) {
  ...
  if (httpOptions.timeout) {
    configuration.timeout = httpOptions.timeout;
  }

  // Set up custom fetch to add telemetry headers
  if (httpOptions.headers && Object.keys(httpOptions.headers).length > 0) {
    const customHeaders = httpOptions.headers;
    configuration[customFetch] = async (url, options) => {
      // Add custom headers to the request, overriding any default headers
  ...
  // Enable implicit flow (id_token) if configured
  ...
  const client = {
    ...
    async callback(redirectUri, params, checks, extras) {
      ...
      // Handle implicit flow (no code, but has id_token)
      if (!callbackParams.has('code') && callbackParams.has('id_token')) {
        ...
        if (isFormPost) {
          ...
          // For implicit flow, tokens come in the URL fragment (hash)
          // Create URL with hash fragment for openid-client to parse
          const urlWithHash = new URL(redirectUrl.href);
          urlWithHash.hash = callbackParams.toString();
          currentUrlOrRequest = urlWithHash;
        }
        ...
        const result = await implicitAuthentication(
          configuration,
          currentUrlOrRequest,
          checks?.nonce,
          v6Checks,
        ...
        return {
          ...
          expires_in: callbackParams.get('expires_in')
            ? parseInt(callbackParams.get('expires_in'), 10)
            : undefined,
          claims: result.claims,
        };
      }

      // Handle authorization code flow and hybrid flow
      if (!callbackParams.has('code')) {
        throw new Error(
          'No authorization code found in callback parameters. ' +
            'For implicit flow, use response_type=id_token. ' +
            'For authorization code flow, ensure the IdP returns a code.',
        );
      }
    ...
    async introspect(token, hint) {
      const parameters = hint ? { token_type_hint: hint } : undefined;
      return await tokenIntrospection(configuration, token, parameters);
    },

    async requestResource(resourceUrl, accessToken, options = {}) {
      const method = options.method || 'GET';
      const body = options.body;

      // Build headers, but strip Authorization as it's handled via accessToken
      let requestHeaders;
      if (options.headers) {
        requestHeaders = new Headers(options.headers);
        // oauth4webapi doesn't allow Authorization header to be passed manually
        // It's added automatically via the accessToken parameter
      ...
      // Return in a format compatible with v4 API
