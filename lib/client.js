...
async function get(config) {
  ...
  // Capture original state BEFORE any modifications to ensure clean restoration
  const originalState = {
  ...
  const customFetchFn = async (url, options = {}) => {
    ...
    let response;
    try {
      response = await originalState.globalFetch(url, fetchOptions);
    } catch (error) {
  ...
  // Create temporary fetch context to avoid race conditions
  // Instead of mutating globals, we'll use a scoped approach
  const withCustomFetch = async (fn) => {
    // Temporarily set custom fetch only for this operation
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        const discoveryResponse = await originalState.globalFetch(discoveryUrl);
