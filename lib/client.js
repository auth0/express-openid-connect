...
async function get (config) {
  const defaultHttpOptions = (options) => {
    options.headers = {
      ...
      ...(config.enableTelemetry ? { 'Auth0-Client': Buffer.from(JSON.stringify(telemetryHeader)).toString('base64') } : undefined)
    };
    options.timeout = 5000;
    return options;
  };
  const applyHttpOptionsCustom = (entity) => {
    entity[custom.http_options] = config.httpOptions ? (...args) => config.httpOptions(defaultHttpOptions(...args)) : defaultHttpOptions;
  };
