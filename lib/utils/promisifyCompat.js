...
function isPromiseReturningMethod(method, context) {
  ...
  // Single-argument non-callback methods: treat as promise-returning to avoid promisify adding extra callback
  if (method.length === 1) {
    const src = method.toString();
    if (!/\b(cb|callback)\b/.test(src)) {
      return true;
    }
  }
  return false;
...
function isPromiseSource(method) {
  try {
    const src = method.toString();
    return (
      /Promise\.(resolve|reject)/.test(src) || /new\s+Promise\s*\(/.test(src)
    );
  } catch {
    return false;
  }
