...
// Helper for binary validation that works in both Node.js and Cloudflare Workers
// Joi.binary() is not available in some environments (e.g., Cloudflare Workers)
const binaryMin8 = (() => {
  // Try to use Joi.binary() if available (Node.js)
  if (typeof Joi.binary === 'function') {
    try {
      return Joi.binary().min(8);
    } catch {
      // Fall through to custom validator
    }
  }
  // Fallback for environments where Joi.binary() is not available
  return Joi.any()
    .custom((value, helpers) => {
      ...
      if (!isBufferLike) {
        return helpers.error('binary.base');
      }
      if (value.length < 8) {
        return helpers.error('binary.min', { limit: 8 });
      }
      return value;
    })
    .messages({
...
const paramsSchema = Joi.object({
  secret: Joi.alternatives([
    Joi.string().min(8),
    binaryMin8,
    Joi.array().items(Joi.string().min(8), binaryMin8),
  ]).required(),
  session: Joi.object({
  ...
  clientSecret: Joi.string()
    .when(
      ...
      {
        is: true,
        // mTLS replaces secret-based auth, so do not require a secret then.
  ...
  clientAssertionSigningAlg: Joi.string()
    ...
    .when('clientAssertionSigningKey', {
      ...
      // Optional when the key already encodes its algorithm:
      // Web Crypto CryptoKey or JWK with an "alg" property.
      is: Joi.custom((value) => {
        if (value == null) throw new Error();
        // CryptoKey: algorithm is embedded in the key itself
        if (
        ...
        // JWK with embedded alg: algorithm is explicit in the JWK
        if (
          typeof value === 'object' &&
          !Buffer.isBuffer(value) &&
          !value.asymmetricKeyType &&
          value.alg
        ) {
          throw new Error();
        }
      ...
      then: Joi.string().required().messages({
        'any.required':
          '"clientAssertionSigningAlg" is required when "clientAssertionSigningKey" is a PEM, Buffer, KeyObject, or a JWK without an "alg" property',
      }),
