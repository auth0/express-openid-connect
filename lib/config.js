...
const paramsSchema = Joi.object().keys({
  ...
  postLogoutRedirectUris: Joi.array().items(Joi.string().uri()).min(0)
});
...
module.exports.get = function(params) {
  ...
  config.postLogoutRedirectUris = config.postLogoutRedirectUris || [];
  ...
  if (!config.postLogoutRedirectUri) {
    config.postLogoutRedirectUri = config.baseURL;
  }
