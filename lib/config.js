...
const paramsSchema = Joi.object().keys({
  ...
  idTokenAlg: Joi.string().valid('RS256','HS256').optional().default('RS256'),
...
module.exports.get = function(params) {
  ...
  if ('HS256' == config.idTokenAlg && !config.clientSecret) {
    throw new Error('"clientSecret" is required for HS256 ID tokens');
  }
