...
const getLogoutToken = require('./hooks/getLogoutToken');
const storeLogoutToken = require('./hooks/storeLogoutToken');
...
const paramsSchema = Joi.object({
  ...
  backChannelLogout: Joi.boolean().optional().default(false),
  backChannelLogoutStore: Joi.object()
    .optional()
    .default(Joi.ref('session.store'))
    .when(Joi.ref('backChannelLogout'), {
      is: true,
      then: Joi.when('session.store', {
        not: Joi.exist(),
        then: Joi.object().required().messages({
          'any.required': `Back-Channel Logout requires a "backChannelSessionStore" (you can also reuse "session.store" if you have stateful sessions).`,
        }),
      }),
    }),
  getLogoutToken: Joi.function()
    .optional()
    .default(() => getLogoutToken),
  storeLogoutToken: Joi.function()
    .optional()
    .default(() => storeLogoutToken),
  baseURL: Joi.string()
    .uri()
    .required()
