...
const paramsSchema = Joi.object({
  ...
  autoRefreshExpired: Joi.boolean().optional().default(false), // TODO: default to true in next major release
  tokenHistory: Joi.boolean().optional().default(false),
  useMrrt: Joi.boolean()
    .optional()
    .default(false)
    .when(Joi.ref('tokenHistory'), {
      is: false,
      then: Joi.valid(false).messages({
        'any.only': '"tokenHistory" must be true when "useMrrt" is true',
      }),
    }),
});
