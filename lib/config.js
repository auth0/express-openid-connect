...
const paramsSchema = Joi.object({
  ...
  session: Joi.object({
    ...
    ).optional().default((parent) => parent.rolling ? (24 * 60 * 60) : false), // 1 day when rolling is enabled, else false
    absoluteDuration: Joi.when(
      Joi.ref('rolling'),
      {
        is: false,
        then: Joi.number().integer().messages({
    ...
    ).optional().default(7 * 24 * 60 * 60), // 7 days,
  ...
  routes: Joi.object({
    ...
    postLogoutRedirectUri: Joi.string().uri({ allowRelative: true }).default('') //_ TODO: find a better name
  }).default().unknown(false)
});
