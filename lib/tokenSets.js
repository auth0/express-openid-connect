...
const TokenSets = {
   ...
   */
  _filterMrrtable(req, authorizationParams) {
    const tokenSets = this.getAll(req);
   ...
   */
  async _findMrrtable(req, authorizationParams = {}) {
    const mrrtable = this._filterMrrtable(req, authorizationParams);
  ...
  async findCompatible(req, authorizationParams = {}) {
    ...
    found = await this._findMrrtable(req, mergedParams);
