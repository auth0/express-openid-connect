...
const verifyCookie = (cookie, value, keystore) => {
  ...
  // Use lastIndexOf to find the signature separator since the value may contain dots
  // (e.g., URLs like "https://tenant.auth0.com" in MCD origin_issuer)
  const lastDotIndex = value.lastIndexOf('.');
  if (lastDotIndex === -1) {
    return undefined;
  }
  const cookieValue = value.substring(0, lastDotIndex);
  const signature = value.substring(lastDotIndex + 1);
  if (verifySignature(cookie, cookieValue, signature, keystore)) {
    return cookieValue;
  }
