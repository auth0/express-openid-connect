...
class ResponseContext {
  ...
  async callback(options = {}) {
    ...
    next = once(next);
    try {
      // Get transaction state first to determine which issuer to use
      ...
      // MCD: Determine issuer - use origin_issuer from transaction or resolve dynamically
      let client, issuer;
      ...
      if (originIssuer) {
        ...
        // MCD: Validate issuer matches what we expected
        if (issuer.issuer !== originIssuer) {
          throw createError(
            400,
        ...
        // No origin_issuer in transaction - use default client (backward compatibility)
