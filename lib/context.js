 ...
 * - client_id, client_secret, client_assertion, client_assertion_type: Client
 *   authentication credentials must be managed through configuration, not request params
 ...
 *
 * These restrictions ensure that security-critical and routing parameters are always set
 * through explicit, typed API parameters rather than untyped extras.
 */
...
function validateTokenExchangeExtras(extra) {
  ...
  return Object.fromEntries(
    Object.entries(extra).filter(([k]) => !PARAM_DENYLIST.has(k)),
  );
}
...
class RequestContext {
  ...
  async customTokenExchange(options = {}) {
    ...
    try {
      return await client.grant(body, {
        clientAssertionPayload: {
          aud: issuer.issuer,
        },
      });
