...
async function refresh({ tokenEndpointParams } = {}) {
  ...
  try {
    ...
    const newTokenSet = await client.refresh(
    ...
    Object.assign(session, {
      ...
      access_token: newTokenSet.access_token || oldTokenSet.access_token,
    ...
    // Add meaningful context for token refresh errors to aid debugging
    const refreshError = new Error(`Token refresh failed: ${error.message}`);
    refreshError.cause = error;

    // Preserve OAuth error properties if they exist
    if (error.error) {
      refreshError.error = error.error;
    }
    if (error.error_description) {
      refreshError.error_description = error.error_description;
    }
...
class RequestContext {
  ...
  get accessToken() {
    try {
      ...
      const { access_token, token_type, expires_at } = ts;
  ...
  async fetchUserInfo() {
    ...
    // Check for temporary tokenSet from callback context first
    let tokens = req.__callbackTokenSet || tokenSet.call(this);
    if (!tokens || !tokens.access_token) {
      throw new Error('No access token available');
    }

    // Get the subject from the ID token claims for v6 compatibility
    let expectedSubject;
    if (req.__callbackTokenSet) {
      // During callback, get subject from the tokenSet directly
      const claims = tokens.claims();
      expectedSubject = claims && claims.sub;
    } else {
      // Normal case, get from existing session
      const idTokenClaims = this.idTokenClaims;
      expectedSubject = idTokenClaims && idTokenClaims.sub;
    }

    return client.userinfo(tokens.access_token, expectedSubject);
  }
}
...
class ResponseContext {
  ...
  async callback(options = {}) {
    ...
    try {
      ...
      if (config.afterCallback) {
        try {
          // Temporarily make tokenSet available for afterCallback context
          req.__callbackTokenSet = tokenSet;
          ...
          // Clean up temporary tokenSet
          delete req.__callbackTokenSet;
        } catch (error) {
          // Clean up temporary tokenSet on error too
          delete req.__callbackTokenSet;

          // Add context for afterCallback errors to aid debugging
          const callbackError = new Error(
            `afterCallback hook failed: ${error.message}`,
          );
          callbackError.cause = error;

          // Preserve all properties from the original error (including status, code, etc.)
          Object.keys(error).forEach((key) => {
            if (key !== 'message' && key !== 'name' && key !== 'stack') {
              callbackError[key] = error[key];
            }
          });
  ...
  async backchannelLogout() {
    ...
    try {
      ...
      // For v6 compatibility: Get JWKS from issuer metadata
      let keyInput;
      if (config.clientSecret && logoutToken.split('.')[0]) {
        // Check if this might be an HS256 token by checking the header
        try {
          const header = JSON.parse(
            Buffer.from(logoutToken.split('.')[0], 'base64url').toString(),
          );
          if (header.alg === 'HS256') {
            keyInput = Buffer.from(config.clientSecret);
          }
        } catch {
          // Fall through to JWKS if header parsing fails
        }
      }
      ...
      if (!keyInput) {
        ...
        if (!issuer.jwks_uri) {
          throw new Error('No jwks_uri found in issuer metadata');
        }

        // Use fetch (should be overridden by client setup)
        const jwksResponse = await fetch(issuer.jwks_uri);
        const jwks = await jwksResponse.json();

        // Import JWKS using JOSE
        const { JWKS } = require('jose');
        keyInput = JWKS.asKeyStore(jwks);
      }

      // Use the trusted issuer from configuration, not from the JWT payload
