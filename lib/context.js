  ...
  async callback(options = {}) {
    ...
    try {
      ...
      try {
        ...
        // Handle stale callback (e.g., user pressed back button after successful login)
        // If no auth verification cookie and user is already authenticated, redirect to baseURL
        if (!authVerification && req.oidc.isAuthenticated()) {
          debug(
            'stale callback detected (no auth verification cookie), user already authenticated, redirecting to baseURL',
          );
