    ...
    try {
      ...
      if (originIssuer) {
        ...
        if (typeof config.issuerBaseURL === 'function') {
          ...
          if (
            normalizeIssuerUrl(originIssuer) !==
            normalizeIssuerUrl(config.issuerBaseURL)
          ) {
            throw createError(
              400,
  ...
  async backchannelLogout() {
    ...
    try {
      ...
      if (typeof config.issuerBaseURL === 'function') {
        ...
        // SSRF Prevention: Only accept backchannel logout from known issuers.
        // An issuer is "known" if a user has previously logged in via that issuer
        // (i.e., it exists in our cache). This prevents attackers from crafting
        // logout tokens with arbitrary issuer URLs to trigger SSRF attacks.
        const issuerManager = getIssuerManager();
        if (!issuerManager.isKnownIssuer(tokenIssuer)) {
          debug('backchannel logout rejected: unknown issuer %s', tokenIssuer);
          res.status(400).json({
            error: 'invalid_request',
        ...
        // Get the client for this known issuer
