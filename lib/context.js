...
const { TokenSets } = require('./tokenSets');
...
/**
 * @returns {Promise<import('..').AccessToken>}
 */
async function refresh({ tokenEndpointParams } = {}) {
  ...
  Object.assign(session, newValues);

  TokenSets.patchByAccessToken(req, oldTokenSet.access_token, newValues);

  // If we received a new RT when refreshing we have to update every tokenset
  // that was bound to this RT since we can't assume it's valid anymore.
  if (newTokenSet.refresh_token) {
    TokenSets.patchByRefreshToken(req, oldTokenSet.refresh_token, {
...
function tokenSet() {
  ...
  if (!('value' in cachedTokenSet)) {
    ...
    if (session.audience) {
      values.audience = session.audience;
    }

    if (session.scope) {
      values.scope = session.scope;
    }

    if (session.organization) {
      values.organization = session.organization;
    }

    cachedTokenSet.value = new TokenSet(values);
  }
...
class ResponseContext {
  ...
  async login(options = {}) {
    ...
    try {
      ...
      // Keep requested audience and organization in the state so we can
      // save them with the tokenset when the callback is invoked later.
      if (options.authorizationParams.audience) {
        stateValue.audience = options.authorizationParams.audience;
      }

      if (options.authorizationParams.organization) {
        stateValue.organization = options.authorizationParams.organization;
      }
  ...
  async callback(options = {}) {
    ...
    try {
      ...
      // Save audience and organization so we can evaluate compatibilities.
      if (req.openidState.audience) {
        session.audience = req.openidState.audience;
      }

      if (req.openidState.organization) {
        session.organization = req.openidState.organization;
      }

      TokenSets.append(req, session);
...
module.exports = { RequestContext, ResponseContext, tokenSet };
