...
class ResponseContext {
   ...
   *
   * NOTE: Currently uses static config.baseURL. For multi-domain deployments where
   * users access the app via different domains (e.g., a.myapp.com, b.myapp.com),
   * the redirect_uri must match the domain the user is on, otherwise:
   * 1. Transaction cookies won't be sent (cookies are domain-specific)
   * 2. Auth0 may reject the callback if redirect_uri isn't registered
   *
   * Workaround for multi-domain apps: Override redirect_uri in login options:
   *   res.oidc.login({
   *     authorizationParams: {
   ...
   *   });
   *
   * Future enhancement: Support baseURL as a function similar to issuerBaseURL.
   *
      ...
      if (originIssuer) {
          ...
          if (originIssuer !== config.issuerBaseURL) {
            throw createError(
              ...
              'Issuer mismatch: transaction was started with a different issuer',
              {
                ...
                error_description: `Expected issuer ${config.issuerBaseURL}, but transaction was started with ${originIssuer}`,
              },
            );
          }
          ({ client, issuer } = await getClient(config));
        ...
        if (
          ...
          throw createError(
            ...
            {
              error: 'issuer_mismatch',
              error_description: `Expected issuer ${originIssuer}, got ${issuer.issuer}`,
            },
          );
        }
      } else {
