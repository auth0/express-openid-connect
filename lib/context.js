  ...
  async callback(options = {}) {
    ...
    try {
      ...
      try {
        ...
        // If no auth verification cookie, redirect to baseURL instead of throwing an error.
        // If user is authenticated, they'll land on the home page.
        // If not authenticated, authRequired middleware will redirect to login if needed.
        if (!authVerification) {
          debug(
            'stale/replayed callback detected (no auth verification cookie), redirecting to baseURL',
          );
