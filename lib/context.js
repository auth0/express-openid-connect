...
const SESSION_EXPIRY_LEEWAY = 30; // seconds
...
async function refresh({ tokenEndpointParams } = {}) {
  ...
  if (
    ...
    now >= session.sessionExpiresAt - SESSION_EXPIRY_LEEWAY
  ) {
...
class ResponseContext {
  ...
  async callback(options = {}) {
    ...
    try {
      ...
      if (claims?.session_expiry != null) {
        const sessionExpiresAt = Number(claims.session_expiry);
        // Lockout guard: reject a session that is already past its ceiling at login.
        if (sessionExpiresAt <= (claims.iat || Math.floor(Date.now() / 1000))) {
          throw createError(
            400,
