...
class RequestContext {
  ...
  async refreshToken() {
    ...
    if (!this.tokens.expired()) { return; }
    const newTokens = await this.client.refresh(this.tokens);
    this._req.session.openidTokens = Object.assign({}, this.tokens, newTokens);
    await this.load();
  }
}
...
class ResponseContext {
  ...
  getRedirectUri() {
    return urlJoin(this._config.baseURL, this._req.baseUrl || '', '/callback');
  }
