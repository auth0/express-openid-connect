...
class RequestContext {
  ...
  async fetchUserInfo() {
    ...
    return oidcClient.fetchUserInfo(configuration, ts.access_token, claims.sub);
  }
...
class ResponseContext {
  ...
  async backchannelLogout() {
    ...
    try {
      ...
      // Get the JWKS cache from configuration
      // openid-client v6 caches JWKS internally after discovery
      const jwksCache = oidcClient.getJwksCache(configuration);

      // If JWKS is cached, use it; otherwise fetch from jwks_uri
      let JWKS;
      if (jwksCache && jwksCache.jwks) {
        JWKS = createLocalJWKSet(jwksCache.jwks);
      } else if (serverMetadata.jwks_uri) {
        // Fallback: fetch JWKS if not cached
        ...
        throw new Error('No JWKS available for token verification');
      }
