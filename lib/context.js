...
const { SessionExpiredError, EnterpriseConnectError } = require('./errors');
  ...
  get accessToken() {
    ...
    if (config.enterpriseConnect) {
      throw new EnterpriseConnectError(
        'accessToken',
        'No refresh tokens are issued in Enterprise Connect mode; extract identity from the ID token in afterCallback instead.',
      );
    }
    try {
  ...
  async requestSessionTransferToken(options = {}) {
    ...
    if (config.enterpriseConnect) {
      throw new EnterpriseConnectError(
        ...
        'Enterprise Connect has no Auth0-managed session to transfer.',
      );
    }
  ...
  async logout(params = {}) {
    ...
    try {
      ...
      const getLogoutUrl = (idTokenHint) => {
        ...
        // `federated` is forced to the literal string 'true' (never ''),
        // since an empty value is silently dropped and the enterprise IdP
        // session would not be terminated.
        return buildEndSessionUrl(config, clientResult, {
          ...config.logoutParams,
          ...(idTokenHint && { id_token_hint: idTokenHint }),
          post_logout_redirect_uri: returnURL,
          ...(params.federated && { federated: 'true' }),
          ...params.logoutParams,
        });
      };
