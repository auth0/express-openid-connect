...
const { get: validateConfig } = require('./config');
...
class ResponseContext {
  ...
  async login (options = {}) {
    ...
    try {
      ...
      // Check that any new `authorizationParams` are valid against the whole config
      validateConfig({
        ...config,
        authorizationParams
      });
      ...
      if (authorizationParams.max_age) {
        transient.store('max_age', req, res, { ...transientOpts, value: authorizationParams.max_age });
      }

      const authorizationUrl = client.authorizationUrl(authorizationParams);
