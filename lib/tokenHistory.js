...
const TokenHistory = {
   ...
   */
  _findMrrtable(tokenSets, authorizationParams) {
    return tokenSets.find(
      /**
       * @param {import('..').TokenSetParameters} ts
   ...
   */
  async _doMrrtRefresh(config, tokenSet, authorizationParams = {}) {
  ...
  async findCompatible(req, authorizationParams = {}) {
    ...
    if (config.useMrrt) {
      ...
      const mrrtable = this._findMrrtable(tokenSets, mergedParams);

      if (mrrtable) {
        found = await this._doMrrtRefresh(config, mrrtable, mergedParams);
